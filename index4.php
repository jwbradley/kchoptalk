...
$ret[0] = $buffer->get('/profiles/526d4a13e350edfd4e0001c9/updates/sent', $data);

$ret[1] = $buffer->get('/profiles/526d4a13e350edfd4e0001c9/updates/pending', $data);
	...
	foreach($ret[$i] as $key=>$row) 
	{
		foreach($row as $key2=>$column)
		{
			foreach($column as $key3=>$value)
			{
				if ($key3 == "statistics")
				{
					$ranker = ($value['retweets']*20) + ($value['reach']) + ($value['clicks']*15) + ($value['favorites']*10) + ($value['mentions']*5);	
					$outputKey[$postCounter]['rank'] = $rankKey[$postCounter] =  $ranker;
					 					
				}
				// if ($value['status'] == 'buffer') {
				// 	$outputKey[$postCounter]['rank'] = 0;
				// }
				// if ($key3 == "created_at")  {
				// 	$outputKey[$postCounter]['created_at'] =  $value;	
				// }
				if ($key3 == "text")
				{
					...
					$href_String = substr($value, strrpos($value, 'http://'), strlen($value)-strrpos($value, 'http://') );
					$outputKey[$postCounter]['link'] = "<p><a href=\"".$href_String. "\"  target=\"_blank\"><strong>".substr($value, 0, strrpos($value, 'http://') )."</strong></a></p>";
					insertrec(nextrec(), $href_String);
				}
...
<td width="20%" valign="top">
	...
	<li class="indent4">  
		<?php
		     	rsort($outputKey);
				// array_multisort($rankKey, $outputkey);
				for($x=0;$x<10;$x++)
				{
					echo $outputKey[$x]['link'];
				}
